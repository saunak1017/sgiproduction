...
const brand=()=>`<a class="brand" href="#/active" aria-label="Shivani Gems home"><div class="brandmark">${icon('diamond')}</div><div><strong>SHIVANI GEMS</strong><span>Production workspace</span></div></a>`;
...
function fileItem(f){const image=['image/jpeg','image/png','image/webp','image/gif','image/avif'].includes(f.mime);return `<div class="file-row">${image?`<a href="/api/files/${f.id}?inline=1" target="_blank" rel="noopener"><img loading="lazy" src="/api/files/${f.id}?inline=1" alt="${esc(f.name)}"></a>`:icon('file')}<a href="/api/files/${f.id}">${esc(f.name)}<small>${fileSize(f.size)}</small></a></div>`;}
...
function openDialog(title,body){
  ...
  $('[data-close]',d).onclick=()=>{if(!state.busy)d.close();};d.addEventListener('cancel',e=>{if(state.busy)e.preventDefault();});d.addEventListener('close',()=>d.remove());d.showModal();return d;
}
